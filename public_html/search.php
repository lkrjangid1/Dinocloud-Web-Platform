...
if(mysqli_num_rows($result) > 0)
{
 ...
 while($row = mysqli_fetch_array($result))
 {
  $output .= '
   <tr>
     <td><img src="http://dinocloud.tk/uploads/' . $_SESSION["userSession"] . '/' . $row["FileName"] . '" width="55px" height="45px"></img></td>
     <td>'.$row["FileName"].'</td>
     <td>'.$row["FileType"].'</td>
     ...
     <td>
        ...
        <button class="btn" data-clipboard-text="https://dinocloud.tk/uploads/' . $_SESSION["userSession"] . '/' . $row["FileName"] . '">
        Share
        </button>
        </img>
     </td>
